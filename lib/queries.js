...
const showAll = (q) => {
    ...
    switch (q) {
        ...
        // Show all roles
        case "Roles":
            dbPool.query('SELECT role.id, title, salary, department.name AS department FROM role INNER JOIN department ON department.id = role.department_id;')
                .then((result) => {
                    // whitespace for proper display of table header
                    console.log(`\n \n`);
                    console.table(result[0]);
        ...
        // Show all employees
        case "Employees":
            dbPool.query('SELECT employee.id, employee.first_name, employee.last_name, role.title, department.name, role.salary, CONCAT(mgr.first_name," ",mgr.last_name) AS manager_name FROM employee INNER JOIN role ON role.id = employee.role_id INNER JOIN department on role.department_id = department.id INNER JOIN employee AS mgr ON mgr.id = employee.manager_id;')
            .then((result) => {
                // whitespace for proper display of table header
                console.log(`\n \n`);
                console.table(result[0]);
...
async function queryBySelection(table, output, param) {
    console.log(`SELECT ${output} FROM ${table} WHERE ${param}`)
...
const getDeptBudget = (dept_id) => {
    dbPool.query('SELECT salary, role.id FROM role INNER JOIN employee ON employee.role_id = role.id WHERE department_id = 3?', dept_id)
