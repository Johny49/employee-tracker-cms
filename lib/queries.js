...
async function buildList(type) {
    switch (type) {
        case 'department':
            dbPool.query('SELECT name FROM department ORDER BY id')
                .then((depts) => {
                    for (dept of depts[0]) {
        ...
        case 'role':
            dbPool.query('SELECT title FROM role ORDER BY id')
                .then((roles) => {
                    for (role of roles[0]) {
        ...
        case 'employee':
            dbPool.query('SELECT CONCAT(first_name," ",last_name) AS name FROM employee ORDER BY last_name ASC')
                .then((employees) => {
...
const updateEmployee = (name, newRole, newMgr) => {
    ...
    dbPool.query('SELECT id FROM role WHERE title=?;', newRole)
        .then((role) => {
            // get id for new manager unless NO MANAGER selected
            if (newMgr !== 'No Manager') {
                ...
                // no manager, set manager_id to NULL
                let noMgr = null;
                dbPool.query('UPDATE employee SET role_id = ?, manager_id = ? WHERE first_name = ? AND last_name = ?;', [role[0][0].id, noMgr, name.split(" ")[0], name.split(" ")[1]])
                    .then((result) => {
                        console.clear();
                        console.log(`Updated record for ${name} in the database.`);
                    })
            ...
            console.log(err);
            console.log(`${name} was not updated.`);
        });
